    ...
    <div className="relative flex min-h-screen w-full flex-col bg-background-light dark:bg-background-dark text-gray-800 dark:text-gray-200 font-display">
        ...
        <div className="flex w-full max-w-6xl items-center justify-between">
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden md:flex items-center gap-8">
              ...
              <Link to={`${base}/collaborators`} className={`text-sm font-medium leading-normal transition-colors ${isActive(base + '/collaborators') ? 'text-white' : 'text-white/80 hover:text-white'}`}>Colaboradores</Link>
              <Link to={`${base}/request`} className={`text-sm font-medium leading-normal transition-colors ${isActive(base + '/request') ? 'text-white' : 'text-white/80 hover:text-white'}`}>Solicitar</Link>
            </nav>
          </div>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-background-dark border-b border-white/10 p-4 flex flex-col gap-4 animate-fade-in">
           ...
           <Link to={`${base}/collaborators`} onClick={() => setIsMobileMenuOpen(false)} className="text-white/80 hover:text-white">Colaboradores</Link>
           <Link to={`${base}/request`} onClick={() => setIsMobileMenuOpen(false)} className="text-white/80 hover:text-white">Solicitar Legenda</Link>
           {isAdmin && (
      ...
      <footer className="w-full border-t border-white/10 mt-auto bg-background-dark px-6 py-8">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-4">
          <p className="text-sm text-white/60">© 2024 Letra na Tela. Todos os direitos reservados.</p>
          <div className="flex items-center gap-4 text-sm">
            <Link to="#" className="text-white/60 hover:text-white transition-colors">Privacidade</Link>
            <Link to="#" className="text-white/60 hover:text-white transition-colors">Termos</Link>
          </div>
        </div>
      </footer>
    </div>
